...
<div class="container">
    ...
    <main>
        ...
        <div class="main">
            ...
            <h2>Реестр адвокатских образований</h2>
            <div class="search_block">
                <input type="search" placeholder="Поиск организации" name="" id="search">
            ...
            <div class="results">
                <table >
                    ...
                    <?php 
                        ...
                        $result = $pdo->query('SELECT * FROM advokats_org ORDER BY name ASC LIMIT 20');
                        $resultf = $result->fetchAll();
                        foreach ($resultf as $row) {
                            print_r('<tr data-id="'.$row[id].'"><td>'.$row[name].'</td><td>'.$row[form].'</td><td>'.$row[contacts].'</td><td>'.$row[director].'</td></tr>');
                        }
                    ?>
                    </tbody>
                </table>
...
<script src="/assets/js/pleader_org.js"></script>
